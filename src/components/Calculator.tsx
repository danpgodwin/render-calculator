...
export default function Calculator() {
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (!selections.area || parseFloat(selections.area) <= 0) {
  ...
  const findBestSystem = (projectType, substrate) => {
    ...
    Object.entries(brands).forEach(([brandKey, brand]) => {
      Object.entries(brand.systems).forEach(([systemKey, system]) => {
        if (system.suitable_substrates.includes(substrate)) {
          const score = system.performance.durability + 
  ...
  const calculateQuantities = (systemData, area) => {
  ...
  const getSubstrateOptions = () => {
    return systemsConfig.project_types[selections.projectType]?.substrates || []
  }
              ...
              <div>
                ...
                <p style={{
                  ...
                  {selections.projectType 
                    ? systemsConfig.project_types[selections.projectType]?.name 
                    : 'Not selected'
                  }
                </p>
              </div>
